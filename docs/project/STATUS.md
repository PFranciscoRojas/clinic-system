...
- `#153` (2026-07-07) chore(legal): remoción del correo personal `franciscorojas92@gmail.com` de todo el contenido legal/consentimientos — reemplazado por `legal@chapni.com` (términos, reembolsos, firma genérica) y `privacidad@chapni.com` (política de privacidad, derechos habeas data) en `content.ts`, `LegalDoc.tsx`, `TermsPage.tsx`, `PrivacyPage.tsx` y el seed `000040_legal_documents.up.sql`. También publicado **directo en la BD de producción** (nueva versión `2026-07-07` de `privacy` y `terms` en `legal_documents`, mismo patrón atómico `is_current=false`→`INSERT is_current=true` del endpoint admin) — versiones previas quedan archivadas para trazabilidad. **Desplegado completo**: CI verde (lint+test+build+deploy+smoke) para core-api, frontend reconstruido manualmente en VPS y verificado en el bundle servido (`app.chapni.com`) — cero ocurrencias del correo personal, `legal@chapni.com`/`privacidad@chapni.com` presentes. Decisión tomada: mantener `hola@chapni.com` como contacto general (no cambiar a `info@chapni.com` — ya es el ancla NAP para SEO local).
- `#152`–`#147` (2026-07-06): fix flujo de grabación de sesión (exit trap, alucinación, re-selección de formato), tag `{collapsed}` + acordeón para secciones poco usadas, fix `time.Until` en vez de `t.Sub(time.Now())` en booking, seed script para tenant `marketing-demo` separado, gate de vouchers Efecty por lead time + atomic slot hold. Detalle no capturado en una sesión de `/actualizar-contexto` propia — ver `git log` para hashes exactos.
