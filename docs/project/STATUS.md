...
- `#164` (2026-07-09) refactor(frontend): **reglas react-hooks `set-state-in-effect` y `exhaustive-deps` promovidas a `error`** — los 32 findings legacy resueltos: syncs de estado derivado convertidos a ajustes en render (seeds de formularios en SuperAdmin/AIDraftPage/RecordForm/AgendaCalendar, trigger de break-the-glass, limpieza de slot pasado) o a inicializadores lazy (`useMediaQuery` → `useSyncExternalStore`, retorno OAuth de GCal, config de bloqueo de pantalla, loading de auth, seed de `BirthDateField`, modal DPA ahora 100% derivado); deps faltantes vía `useCallback` o añadidas. Los 7 efectos con set síncrono restantes (restores one-shot, resets de ciclo de fetch) llevan disable acotado con justificación inline. Verificado con smoke de navegador real contra prod (login + 7 rutas).
- `#163` (2026-07-09) fix(clinical): **visor de borrador bloqueado cubre plantillas personalizadas** — el restore bloqueado solo capturaba `{uiType, draft}`; los registros con plantilla custom guardan su contenido en `customSections`, así que el visor mostraba un formulario vacío. Ahora captura secciones + template id, renderiza vía `TemplatedSectionsForm` (fallback clave/valor si la plantilla fue borrada) y ofrece "Recuperar con su formato". Además: seed `000040` rebrandeado SGHCP→Chapni (29 reemplazos, rol `sghcp_app` intacto), correo personal purgado de `RFC-001`, backlog marcado.
- `#162` (2026-07-09) fix(clinical): **guarda estructural anti prompt-injection en el pipeline de IA** — cierra la Fase 3 (y con ella toda la auditoría 360°): `prompt_guard.py` envuelve transcripción/historia en `<transcripcion>`/`<historia_clinica>`, neutraliza tags embebidos (fullwidth `＜`) y los 4 system prompts anclan su regla de "solo datos" al tag. Tests nuevos (11/11).
- `#161` (2026-07-09) fix(frontend): **eliminado el scroll horizontal de página en móvil** — causa raíz hallada empíricamente (Playwright a 393px contra prod, logueado con el tenant demo Consultorio Aurora): grids de layout con `'1fr'` a secas (= `minmax(auto,1fr)`) crecían al min-content de una fila de cita (~457px) y arrastraban toda la página; tabs de Facturación sin wrap (452px). Fix: `minmax(0,1fr)` en Dashboard/AIDraft/Appointment/NewAppointment, `flexWrap` en tabs, y `<main>` del AppShell con `overflowX: hidden` como respaldo sistémico (las tablas anchas conservan su scroll interno en `.table-wrap`). Verificado post-deploy: `main.scrollWidth` = 393 en las 7 rutas.
- `#160`–`#159` (2026-07-09): actualización de STATUS (fases de auditoría verificadas contra código) + auditoría responsive inicial (filas de `ClinicalPage` como tarjetas apiladas en móvil, header de `ClinicalRecordPage` con wrap).
- **Repo `../chapni`** (`b9c6fd7`, 2026-07-09): **hub `/recursos` construido y desplegado** — el mayor multiplicador SEO/GEO pendiente del plan. Colección Astro con 4 piezas (guía Res. 1995 historia clínica, nota SOAP con plantilla, habeas data Ley 1581, plantilla de consentimiento informado con autorización de datos), índice por categoría, artículo con schema Article+FAQPage+BreadcrumbList, Nav/Footer enlazados. Live en `chapni.com/recursos/` (Cloudflare, sitemap regenerado). Copy según reglas de marca (sin guion largo, sin negrita decorativa).
- Sesión 2026-07-09 noche (content-ops, sin PRs en este repo — commits `4515d7b..5e58930` en `../chapni` + skill local): **sistema completo de operación de contenido social montado y probado end-to-end**. La skill `chapni-social` ganó paso 0 de auditoría (comandos `estado`/`semana`), sinergia con el hub `/recursos` (pilar Educativo se alimenta de las guías, CTA intermedio a la guía en vez del signup), política de slots perdidos y ritual semanal dominical en batch. El content-log se movió al repo chapni (`docs/marketing/content-log.md`, columna Publicado ⬜/✅/❌). Rutina cloud `trig_01Brer4kRkziJPdUesVNnQ9k` (domingos 8am Bogotá) lee el log, audita la semana y deja el reporte como borrador en Gmail — GitHub App instalada en chapni+clinic-system para esto. Perfiles sociales terminados: FB con URL corta `facebook.com/chapniapp` y NAP completo, LinkedIn corregido (tipo de empresa, lema con keywords, About reescrito con la regla "la IA sugiere, tú firmas"), banners oficiales de FB y LinkedIn generados (`scripts/render_banner.py` nuevo en la skill) y subidos. Contenido real: post de jueves (LinkedIn Producto, publicado ✅) y viernes (IG+FB Motivacional, programado — primer post del perfil de IG).
- Sesión 2026-07-09 (operativo): **branch protection activada en `main`** (require PR, sin push directo ni force-push, `enforce_admins`, 0 approvals para conservar el flujo de squash-merge propio) · **contraseña del demo `consultorio-aurora` reseteada** a `Marketing1234!` (email `franciscorojas92+aurora@gmail.com`) para diagnóstico con navegador real · tareas_clinica y fases de auditoría verificadas contra código.
