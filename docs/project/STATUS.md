...
- **Sin commitear** (2026-07-07): remoción del correo personal `franciscorojas92@gmail.com` de todo el contenido legal/consentimientos — reemplazado por `legal@chapni.com` (términos, reembolsos, firma genérica) y `privacidad@chapni.com` (política de privacidad, derechos habeas data) en `content.ts`, `LegalDoc.tsx`, `TermsPage.tsx`, `PrivacyPage.tsx` y el seed `000040_legal_documents.up.sql`. Publicado también **directo en la BD de producción** (nueva versión `2026-07-07` de `privacy` y `terms` en `legal_documents`, mismo patrón atómico `is_current=false`→`INSERT is_current=true` del endpoint admin) — versiones previas quedan archivadas para trazabilidad, ya no exponen el correo personal. **Pendiente:** commit local + rebuild manual de frontend en VPS (el footer hardcoded de `TermsPage.tsx`/`PrivacyPage.tsx` aún no está desplegado; la BD ya está corregida). Decisión tomada: mantener `hola@chapni.com` como contacto general (no cambiar a `info@chapni.com` — ya es el ancla NAP para SEO local).
