...
- **ESLint: subir reglas react-hooks de `warn` a `error` (2026-07-02)** — al montar el gate de eslint (PR #110) había 43 findings pre-existentes, todos de las reglas nuevas de `eslint-plugin-react-hooks` v6+ (`set-state-in-effect` ×24, `exhaustive-deps`, `purity`, `static-components`, `refs`) más `no-explicit-any` ×6. Quedaron en `warn` en `eslint.config.js` para no chocar con el rediseño frontend WIP. Cuando el WIP aterrice: arreglar los findings y subir esas 5 reglas a `error`.
