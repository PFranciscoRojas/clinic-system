...
Widgets are fully retired from active templates (migration 000067) and only
survive in ARCHIVED template versions that in-progress drafts may still be
anchored to. None of them is AI-fillable anymore:

  - mental_exam: became generic select/multiselect fields; the system prompt
    forbids the AI from marking exam options (la IA sugiere, el humano decide).
  - risk: replaced by the fixed risk_level system control — the AI suggestion
...
validate_widget_value() returns the value when it may pass through, or None
...
_MANUAL_ONLY_WIDGETS = {"mental_exam", "risk", "treatment_plan", "diagnoses"}
