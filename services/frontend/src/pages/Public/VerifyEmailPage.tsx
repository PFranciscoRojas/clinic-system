...
// Public landing reached from the signup verification email
// (/verify-email?token=…). No auth: the single-use token is the credential.
// It auto-submits on mount and then sends the user to login.
export function VerifyEmailPage() {
  ...
  const [err, setErr] = useState(token ? '' : 'El enlace no es válido. Crea tu cuenta de nuevo o solicita un enlace.');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr(/inválid|expir/i.test(msg) ? 'El enlace es inválido o expiró.' : 'No se pudo confirmar tu correo.');
        setState('error');
      }
    })();
  }, [token]);
    ...
    <div style={{ minHeight: '100dvh', background: 'linear-gradient(135deg, #2a2769, #171533)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 22, color: '#fff' }}>Confirmación de correo</div>
        </div>
        ...
        <div style={card}>
          {state === 'verifying' && (
            <div style={{ padding: '12px 0' }}>
              ...
              <div style={{ fontSize: 14, color: 'var(--s500)' }}>Confirmando tu correo…</div>
            </div>
          )}
          ...
          {state === 'done' && (
            <div style={{ padding: '8px 0' }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 17, color: 'var(--s800)', marginBottom: 10 }}>¡Correo confirmado!</div>
              <div style={{ fontSize: 13.5, color: 'var(--s500)', lineHeight: 1.7, marginBottom: 24 }}>
                Tu cuenta está activa. Ya puedes iniciar sesión y empezar a configurar tu consultorio.
              </div>
